...
contract iETH is Base {
    ...
    function _getCurrentCash() internal view override returns (uint256) {
        return address(this).balance - openCash;
    }
        ...
        settleInterest
    {
        _redeemInternal(
            _from,
            _redeemiToken,
            _redeemiToken * (_exchangeRateInternal())
        );
    }
        ...
        settleInterest
    {
        _redeemInternal(
            _from,
            _redeemUnderlying / (_exchangeRateInternal()),
            _redeemUnderlying
        );
    }
    ...
    function borrow(uint256 _borrowAmount)
        ...
        settleInterest
    {
        _borrowInternal(payable(msg.sender), _borrowAmount);
    }
        ...
        settleInterest
    {
        ...
        if (openCash > 0) payable(msg.sender).transfer(openCash);
    }
    ...
    function balanceOfUnderlying(address _account) external returns (uint256) {
        ...
        return _exchangeRateInternal() * (balanceOf[_account]);
    }
    ...
    function supplyRatePerBlock() external view returns (uint256) {
        ...
        uint256 _underlyingScaled = totalSupply * (_exchangeRateInternal());
        if (_underlyingScaled == 0) return 0;
        uint256 _totalBorrowsScaled = totalBorrows * (BASE);

        return borrowRatePerBlock() * (
                BASE - (reserveRatio) ) * (
                _totalBorrowsScaled / (_underlyingScaled)
            );
    }
