...
abstract contract Base is TokenERC20 {
    ...
    function _updateInterest() internal virtual override {
        ...
        if (block.number != accrualBlockNumber) {
            ...
            _vars.blockDelta = _vars.currentBlockNumber - accrualBlockNumber;
             ...
             */
            _vars.simpleInterestFactor = _vars.borrowRate * _vars.blockDelta;
            _vars.interestAccumulated = _vars.simpleInterestFactor * _vars.totalBorrows;
            _vars.newTotalBorrows = _vars.interestAccumulated + _vars.totalBorrows;
            _vars.newTotalReserves = reserveRatio * _vars.interestAccumulated + _vars.totalReserves;

            _vars.borrowIndex = borrowIndex;
            _vars.newBorrowIndex = _vars
                .simpleInterestFactor * _vars.borrowIndex + _vars.borrowIndex;
    ...
    function _redeemInternal(
         ...
         */
        _doTransferOut(payable(msg.sender), _redeemUnderlyingAmount);
        ...
        virtual
    {
        ...
        borrowSnapshot.principal = _borrowBalanceInternal(_borrower) + _borrowAmount;
    ...
    function _repayInternal(
        ...
        BorrowSnapshot storage borrowSnapshot = accountBorrows[_borrower];
        borrowSnapshot.principal = _accountBorrows - _actualRepayAmount;
        borrowSnapshot.interestIndex = borrowIndex;

        totalBorrows = totalBorrows < _actualRepayAmount
            ? 0
            : totalBorrows - _actualRepayAmount;
        ...
        returns (uint256)
    {
        ...
        return
            borrowSnapshot.principal * borrowIndex.divup(
                borrowSnapshot.interestIndex
            );
    }
    ...
    function _exchangeRateInternal() internal view virtual returns (uint256) {
        if (totalSupply == 0) {
            ...
            return
                (_getCurrentCash() + totalBorrows - totalReserves).rdiv(
                    totalSupply
                );
        }
    }
